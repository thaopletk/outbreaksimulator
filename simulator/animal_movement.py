...
def animal_movement(
    properties, n, movement_frequency, movement_probability, movement_prop_animals, day
):
    added_animals = []
        ...
        if not (
            ...
            if not properties[premise_index].culled_status:
                ...
                if prob_movement < movement_probability:
                    ...
                    # TODO - to add a limit to how far the animals move (i.e. some 1000 kilometers, for example)
                    moving_to_premise_indices = []
                    for i in range(n):
                        if (
                            not properties[i].culled_status and i != premise_index
