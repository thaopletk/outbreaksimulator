...
class DiseaseSimulation:
    ...
    def simulate_outbreak_spread_only(self, properties, time=0, stop_time=7):
        ...
        while time < stop_time:
            ...
            for i, property_i in enumerate(properties):
                property_i.infection_model(
                    self.latent_period, self.infectious_period, self.preclinical_period, FOI[i], time
                )
            ...
            movement_record = animal_movement.trialsimex_animal_movement(
                properties, day=time, controlzone=controlzone_movement_restrictions
            )
            if movement_record != []:
            ...
            if self.plotting:
                ...
                with open(os.path.join(self.folder_path, "plotting_data" + str(time)), "wb") as file:
                    pickle.dump([properties, time, self.xlims, self.ylims, controlzone, contacts_for_plotting], file)
        ...
        save_movement_record(self.folder_path, self.movement_records)
